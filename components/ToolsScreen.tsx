...
import { listTools, callTool } from '../services/api';
import { ToolDefinition } from '../types';

const ToolCard: React.FC<{ tool: ToolDefinition; onTest: (tool: ToolDefinition) => void }> = ({ tool, onTest }) => (
    <div className="bg-[#0B0C0D]/50 backdrop-blur-lg border border-[#6A4C32]/50 rounded-xl p-6 flex flex-col">
        <h3 className="text-xl font-bold text-[#FEE9B2]">{tool.name}</h3>
        <p className="text-sm text-[#D4A574] mt-2 flex-grow">{tool.description}</p>
        <div className="mt-4">
            <span className="text-xs font-semibold text-[#FFCC66]">PARAMETERS: {tool.params.length > 0 ? tool.params.map(p => p.name).join(', ') : 'None'}</span>
        </div>
...
const ToolsScreen: React.FC = () => {
    ...
    const handleRun = async () => {
        ...
        setIsRunning(true);
        const result = await callTool(selectedTool.name, params);
    ...
    return (
        <div className="p-8 h-full flex flex-col md:flex-row gap-8">
            ...
            <div className="md:w-1/2 lg:w-1/3 bg-[#0B0C0D]/50 border border-[#6A4C32]/50 rounded-xl p-6 flex flex-col">
                ...
                {selectedTool ? (
                    <div className="flex flex-col flex-grow">
                        ...
                        <div className="my-4 space-y-4">
                            {selectedTool.params.map(p => (
                                <input
                                    ...
                                    className="w-full px-4 py-2 bg-[#0F1113] border border-[#6A4C32] rounded-lg text-[#FEE9B2] placeholder-[#6A4C32] focus:outline-none focus:ring-1 focus:ring-[#FFCC66]"
                                />
                            ))}
                        </div>
                        ...
                        <div className="mt-4 flex-grow">
                            ...
                            <pre className="mt-2 p-4 bg-[#0F1113] rounded-lg text-sm text-[#4DEEEA] whitespace-pre-wrap overflow-auto h-48">{output}</pre>
                        </div>
                    </div>
                ) : (
